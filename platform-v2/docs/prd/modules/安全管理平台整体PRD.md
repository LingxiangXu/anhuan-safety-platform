...
发现隐患并登记
        ↓
受理、分派（本级无法处理时上报后由上级直接分配或提出意见交回原受理人分配）、整改、复查、闭环
        ↓
...
| 二级单位领导 | 按等级办理本单位风险最终审批；受理指定给本人的隐患上报，直接定级分配并承担复核，或提出意见交回原受理人分配 |
| 二级单位安全管理人员 | 审核全部风险点；一般/低审核后生效，较大/重大报二级单位领导；受理随手拍及 OA 批准的监督隐患，可直接分配或上报；作为指定上级可直接分配或提意见交回；本人最终分配的隐患由本人复核 |
| 作业区负责人 | 审核本区域风险点、维护巡检计划和现场组织；受理巡检隐患，直接定级分配或上报；依据上级交回意见完成分配；只复核本人最终分配的隐患 |
| 班组长 | 填报风险新增、变更和复评，接收退回并修改重提；关联风险点编制和发布检查标准；协调巡检、隐患整改和现场作业 |
| 岗位人员 | 执行巡检、上报隐患；作为实际整改人提交反馈或按复核驳回意见重新整改；承担授权作业负责人或监护职责 |
...
flowchart LR
  ...
  hazard --> acceptHazard["作业区负责人受理并定级分派"]
  acceptHazard -->|"可处理"| assignHazard["定级分配，绑定最终分配人"]
  acceptHazard -->|"不能处理，上报"| upperHazard["指定上级受理人"]
  upperHazard -->|"直接分配"| assignHazard
  upperHazard -->|"提出意见并交回"| originalHazard["原受理人依据意见分配"]
  originalHazard --> assignHazard
  assignHazard --> rectify["实际整改人反馈"]
  rectify --> hazardReview["最终定级分配人复核"]
  hazardReview -->|"不通过，驳回整改"| rectify
  hazardReview -->|"通过"| closed["隐患关闭"]
...
检查结果只允许合格、不合格；说明和照片为两个独立字段，不合格时说明必填，照片是否必拍按检查标准逐条配置。提交结果后先关闭执行弹框并重新查询任务；存在不合格且未转换项时，自动打开与任务列表“转隐患（N）”相同的弹框，不存在时回到任务列表。每条不合格检查结果分别生成一条隐患，隐患标题直接取对应巡检说明；同一任务存在多条不合格结果时，可勾选单条或多条处理。提交后自动弹框和任务列表入口共用同一待转清单，关闭弹框不丢失待转状态；“查看结果”展示全部检查项及隐患编号，转隐患页面只展示不合格且未转换的检查项。系统按“巡检任务 + 检查项”防止重复建单；隐患关闭不改变原巡检结果。巡检、随手拍、集团专业检查、集团监督检查四类来源在隐患主单中分流记录：普通隐患受理可以“受理并分派”、填写原因后“不受理关闭”，或在本级无法处理时“上报”（见 §4.2a）；整改反馈只填写一项整改情况说明并上传整改后照片；复核由最终定级分配人办理。集团专业检查一期使用独立新增页面，完整登记发现、整改、闭环和考核信息后直接形成已闭环台账，不展示或发起 OA，仅允许同一专业部门查看；集团监督检查跨系统调用 OA 内置 BPM 流程启动入口并完整委托 OA 办理，批准回传考核人和考核金额后进入受理整改；巡检隐患不计奖励，随手拍奖励规则本轮保持不变。

### 4.2a 隐患受理上报与复核责任

隐患登记后按来源确定首次受理人。受理人可直接定级分配；无法处理时填写原因、需解决事项并选择上级受理人，状态为“待上级受理”。上级仅有两种操作：直接定级分配，或提出处理意见并交回原受理人定级分配。不另设独立协调、多级继续上报、退回补充材料或整改人申请协调分支。

谁最终定级分配，谁负责复核：上级直接分配的由上级复核；上级提出意见交回、原受理人完成分配的，由原受理人复核。系统分别记录首次受理人、当前受理人、最终分配人和整改人；复核人自动等于最终分配人，不固定作业区负责人，也不是原 OA 审批人。

实际整改人提交整改说明及整改后照片后为“待复核”；最终分配人填写意见，通过则闭环，驳回则交原整改人重新整改，再由同一最终分配人复核。历史反馈、照片及意见按次保留，不自动改等级或期限；督办不代替复核。

受理上报前不要求先填等级、整改人或期限；实际分配时必须补齐等级、责任部门、整改人、有效期限和整改要求。提出意见交回时意见必填，原受理人看到意见后完成分配。人员来自 OA 同步在职目录并校验平台业务岗位及组织权限；只有当前实际办理人能执行对应动作，重复请求不重复建单或待办。

平台审批相关流转由系统内置 BPM 统一管理，不由隐患模块自建审批流。集团专业检查直接登记已闭环台账不适用；集团监督检查仍先委托 OA，批准后才进入受理，上报不再额外发起 OA。详见隐患模块 PRD §7.3.1a。
...
| 隐患治理 | `/hazards` | 台账、详情、受理上报、上级意见、定级分配、整改反馈、最终分配人复核及完整时间线 |
...
| 隐患 | `hazard`、`hazard_action`、`hazard_rectification`、`hazard_report` | 隐患、上报记录、整改反馈历史、最终分配人及复核待办、督办和驾驶舱 |
...
4. 隐患治理：登记、受理、上报（上级直接定级分配或提出意见交回）、整改反馈、最终分配人复核、关闭或驳回重新整改。
...
| AC-04b | 隐患受理与定级分配 | 正式等级仅一般或重大；登记及未定级即上报时可显示未定级，实际分派前必须确认；不能出现较大或特别重大，上报不自动改等级 |
| AC-05 | 隐患受理、整改、复查退回再整改、关闭 | 时间线完整，状态不可跳跃；另按 AC-05a—AC-05d 验证受理上报分支 |
| AC-05a | 受理人直接分配或上级直接分配 | 记录实际最终分配人，整改后复核待办只交该人，不固定作业区负责人 |
| AC-05b | 上级提出意见并交回 | 意见必填，原受理人依据意见定级分配并承担复核；不显示继续上报、退回补充材料 |
| AC-05c | 复核驳回再整改 | 原整改人再次反馈，历史反馈及照片保留，仍交最终分配人复核；不自动改期限，越权被拦截 |
| AC-05d | 来源与状态限制 | 专业检查直接闭环、OA 审批中、整改中、待复核、结束状态均无受理上报；重复提交不重复生成业务或待办 |
| AC-06 | 逾期隐患生成督办建议并经授权发起 | 督办和隐患状态独立但可互查；待上级受理不暂停原期限计时 |
...
| 隐患治理与督办 | 当前原型已演示四来源分流、不受理关闭、受理分派、整改情况说明与后照片、最终定级分配人复核、专业检查直接闭环台账和独立督办；受理上报分支按 AC-05a—AC-05d 单独验收，不能以已有主线演示替代专项验收；生产后端和独立单据流转待建设 |
