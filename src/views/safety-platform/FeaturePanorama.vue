...
children: ['作业票申请', '资格校验', '分级审批链', '监护确认', '完工验收', '归档'],
features: [
            { icon: '🚦', label: '资格准入闸门', desc: '提交作业申请时系统自动校验人员证件有效期，不满足条件直接阻断' },
  { icon: '🔍', label: '分级审批链', desc: '公司安环人员核验作业方案、安全措施与风险控制，并按「类型+级别」逐级加签至分管领导/集团' },
  { icon: '🛡️', label: '监护确认', desc: '现场监护人通过APP逐项确认安全条件后允许开工' },
  { icon: '📋', label: '分级审批链', desc: '申请→资格校验→分级审批链→监护确认→作业执行→完工验收→归档' }
          ]
        },
        {
